...
- **Frontend**: React(TypeScript, Vite, TanStack Query/Table), Flutter(Dart, PWA+Android), Next.js(App Router)
...
> 법무법인 법승에서는 **CRM · 메인 홈페이지 V2 · 사내 데이터(구글시트→PostgreSQL) 이관**을 스크래치부터 1인 풀스택으로 구축했고, **사건관리 앱**은 2인 팀 개발로 상담 · 문서 · 전자결재 · 정산 도메인과 인프라/배포를 담당했습니다. **레거시 법무법인 홈페이지**는 외주 유지보수 체계를 사내로 전환해 직접 운영 · 개선 중입니다. 이전 직장(리얼타임테크)에서는 실시간 CCTV 모니터링 시스템 유지보수, 신규 개발을 위한 기존 코드 분석, 센서데이터 시각화 시스템 유지보수를 담당했습니다.
...
- **기술스택**: FastAPI(Python) + SQLAlchemy + Pydantic / React + TypeScript + Vite(TanStack Query, TanStack Table, shadcn/ui) / PostgreSQL(Cloud SQL, `postgres_fdw`) / APScheduler / Google Gmail·Drive·Workspace API / GCP Cloud Run
...
**핵심 흐름**: React 화면 → API 라우터 → CRUD → PostgreSQL. 변경사항은 SQLAlchemy 세션 훅에서 감사로그와 SSE 이벤트로 연결합니다. CRM 원격 데이터는 인라인 조인 대신 배치 조회하며, 요청 단위 커밋 이후의 실행취소는 별도 Undo 스택으로 처리합니다.
...
- **구현**: Gmail 결재요청 메일 + **서명토큰 기반 무인증 원클릭 결재 페이지**(비밀번호 없이 토큰 검증만으로 승인/반려), 헤더 알림벨 · 폴링 · 토스트로 구성된 인앱 알림까지 결재 워크플로(요청→승인→반려→재상신)를 구축. 결재 참조자(Cc) 기능은 JSONB(`cc_approver_keys`) + GIN 인덱스로 설계해 별도 상태 컬럼 없이 `approval_status`를 미러링(상태 이원화로 인한 불일치 방지).
- **시도와 롤백**: 페이지 이동 없이 Gmail 안에서 바로 승인/반려하는 **AMP for Email 인이메일 결재**를 구현했으나, 최종 점검에서 AMP 발신자 등록 요건인 **DKIM · DMARC가 도메인에 설정되어 있지 않아** 등록 전까지는 전 수신자가 HTML 폴백만 받는다는 사실을 확인. "동작하지 않을 코드를 남기지 않는다"는 원칙으로 AMP 관련 코드를 전량 롤백하고, 링크 메일 + 인앱 결재 방식으로 운영을 유지.
...
**6) 프론트엔드 공통화 — TanStack Table 기반 공통 DataTable**
- `@tanstack/react-table` 기반 공통 DataTable 컴포넌트를 도입해 **미수금 · 열람등사 · 계약 · 상담료 4종 목록을 일원화**. 컬럼 폭 · 순서 · 표시 · 정렬을 `tableId` 단위로 localStorage에 저장해 사용자별 설정을 유지하도록 구현.

**7) 요청 단위 자동 커밋 구조에서의 동시성 · 일관성 제어 (실제 메커니즘)**
- 이 시스템은 FastAPI 요청 단위로 트랜잭션이 즉시 커밋되는 구조라, DB 트랜잭션 롤백만으로는 실행취소가 불가능함. 따라서 **애플리케이션 레벨에서 역동작 클로저를 스택으로 쌓는 방식의 Undo 기능**을 별도로 설계 · 구현.
