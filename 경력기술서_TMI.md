...
- 전문센터의 히어로·본문·브랜드 스타일을 컴포넌트와 콘텐츠 데이터로 공통화.
- **근거**: [8/26](daily/8월/2026-08-26.md) `ac81362`, `0fec4fc`; [6/1](daily/6월/2026-06-01.md) `389171c`.

**프론트엔드 C) 반응형 어드민 UI — 표·카드 전환 기준 직접 수립**
- **문제**: 어드민 목록 12개가 모두 `<table>` + `overflow-x-auto` 구조라, 열이 최대 9개인 화면은 좁은 폭에서 스크롤해도 내용을 읽을 수 없었음. 기존에 정해진 모바일 대응 기준이 없어(grep 0건) 화면마다 제각각 대응하던 상태.
- **해결**: **md(768px) 미만은 카드, 이상은 표**로 전환 기준을 직접 세우고 지사·약관·계정·변호사·센터·문의·게시물 등 12개 화면에 순차 적용. 정렬 화살표·체크박스 터치 영역을 22px·16px에서 36px·44px로 확대하고 삭제 동작은 `order-last`로 모바일 하단에 분리. 태블릿 구간(640~1024px)에서 저장 바가 셸보다 좌우 8px씩 밀려나던 여백 불일치도 마진 규칙을 맞춰 수정.
- **근거**: [9/2](daily/9월/2026-09-02.md) `7c24c77`, `8388008`.
...
- **담당 범위**: 기존 대고객 홈페이지([lawwin.co.kr](https://lawwin.co.kr))의 화면 개선, 검색엔진 대응, 백엔드 API 개발, 소스 이관 및 배포 환경 분석. 외주 소스를 사내 저장소로 이관하고 운영 절차를 문서화해 직접 유지보수할 수 있는 기반을 마련.
